...
esp_err_t WebSocketControlServer::RegisterUri(const httpd_uri_t* handler) {
    if (server_handle_ == nullptr || handler == nullptr) {
        return ESP_FAIL;
    }
    return httpd_register_uri_handler(server_handle_, handler);
}
